#!/bin/bash
# Regenerate all 22 narration MP3s using ElevenLabs API.
# Uses voice nzFihrBIvB34imQBuxub by default (set ELEVEN_VOICE_ID to override).
#
# Setup (one-time):
#   1. Get your ElevenLabs API key: https://elevenlabs.io → profile → API Keys
#   2. Export it in your shell:
#        export ELEVEN_API_KEY="sk_xxxxxxxxxxxxxxxxxxxx"
#      (or add to ~/.zshrc to persist)
#   3. Make sure the voice ID is added to YOUR ElevenLabs voice library:
#        https://elevenlabs.io/app/voice-library?voiceId=nzFihrBIvB34imQBuxub
#        Click "Add to my voices" so the API can access it.
#
# Usage:
#   ./regen_voice_elevenlabs.sh
#
# Optional environment overrides:
#   ELEVEN_VOICE_ID   — different voice ID
#   ELEVEN_MODEL      — eleven_multilingual_v2 (default) or eleven_turbo_v2_5 (faster, cheaper)
#   STABILITY         — 0.0 - 1.0 (default 0.40)
#   SIMILARITY        — 0.0 - 1.0 (default 0.80)
#   STYLE             — 0.0 - 1.0 (default 0.30)
#
# After this script finishes, re-run ./build_video.sh to rebuild the MP4.

set -euo pipefail

API_KEY="${ELEVEN_API_KEY:-}"
VOICE_ID="${ELEVEN_VOICE_ID:-nzFihrBIvB34imQBuxub}"
MODEL="${ELEVEN_MODEL:-eleven_multilingual_v2}"
STABILITY="${STABILITY:-0.40}"
...
for n in "${SCENE_NUMS[@]}"; do
  ...
  payload=$(jq -nc \
    ...
    --argjson style "$STYLE" \
    '{text: $text, model_id: $model, voice_settings: {stability: $stability, similarity_boost: $similarity, style: $style, use_speaker_boost: true}}')

  http_code=$(curl -sS -o "$out_mp3" -w "%{http_code}" \
  ...
  if [[ "$http_code" == "200" ]]; then
    size=$(stat -f%z "$out_mp3" 2>/dev/null || stat -c%s "$out_mp3")
    if [[ "$size" -lt 1000 ]]; then
      echo "❌ (empty response, see file for error)"
      cat "$out_mp3"; echo ""
      exit 1
    fi
    echo "✓"
...
echo ""
echo "✓ Done. ${#SCENES[@]} narration files regenerated with ElevenLabs voice $VOICE_ID"
echo ""
echo "Now rebuild the video:"
echo "  ./build_video.sh"
