#!/bin/bash
# Regenerate all 22 narration MP3s using macOS's built-in `say` command.
# This produces much better quality than the espeak placeholder.
#
# RECOMMENDED VOICES (free, but you may need to download them first via:
#   System Settings → Accessibility → Spoken Content → Manage Voices)
#
#   - Tom (Premium)        ← DEEP American male, best for true-crime narration
#   - Evan (Premium)       ← American male, slightly higher
#   - Aaron (Enhanced)     ← American male
#   - Daniel (Enhanced)    ← British male, deeper
#
# Usage:
#   ./regen_voice_mac.sh "Tom (Premium)"
#   ./regen_voice_mac.sh "Evan (Premium)"
#   ./regen_voice_mac.sh           # uses default voice
#
...
VOICE="${1:-Tom (Premium)}"
RATE="${RATE:-180}"   # words per minute (default 180; lower = slower)
...
if ! say -v '?' | awk -F '#' '{print $1}' | sed 's/[[:space:]]*$//' | grep -Fxq "$VOICE"; then
  echo "❌ Voice not installed: $VOICE"
  echo ""
  echo "Available US English voices on this Mac:"
  say -v '?' | grep "en_US" | awk -F '#' '{print "  - "$1}' | sed 's/[[:space:]]*$//' | sort -u
  echo ""
  echo "To download premium/enhanced voices:"
  echo "  System Settings → Accessibility → Spoken Content → Manage Voices..."
  exit 1
fi

echo "Using voice: $VOICE  (rate: $RATE wpm)"
echo "Output: $OUT"
echo ""

# Scene narrations — keep identical to scene_narration.py
declare -A SCENES
...
for n in $(seq -f "%02g" 1 22); do
  text="${SCENES[$n]}"
  aiff_path="$OUT/VO_S${n}.aiff"
  mp3_path="$OUT/VO_S${n}.mp3"

  echo "  → VO_S${n}.mp3"
  say -v "$VOICE" -r "$RATE" -o "$aiff_path" "$text"
  ffmpeg -y -loglevel error -i "$aiff_path" -codec:a libmp3lame -qscale:a 4 "$mp3_path"
  rm "$aiff_path"
done

echo ""
echo "✓ Done. ${#SCENES[@]} narration files regenerated with voice: $VOICE"
echo ""
